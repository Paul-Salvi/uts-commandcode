...
# api-pagination
- Paginated list endpoints should accept `page`, `pageSize`, `status`, `search` query params and return `{ records, totalCount, page, pageSize }`. Apply server-side filtering (by status, search text) in the repository layer via LINQ `Where` clauses before applying skip/take pagination — do not fetch all rows from the database just to paginate on the client. Concrete contract: `page` defaults to 1, `pageSize` defaults to 50 and is clamped to 1–100; repositories clamp (`Math.Max(1, page)`, `Math.Clamp(pageSize, 1, 100)`) and use `AsNoTracking()` + `Skip`/`Take` → SQL `LIMIT`/`OFFSET`. Detail/single-entity/bounded endpoints (a GET by id, `FileDeliveries/status`, dashboard summary) are deliberately NOT paginated. Confidence: 0.85
- When paginating a public list endpoint, add a paged variant to the repository/service interface alongside the existing full-list method (e.g., `GetPagedByUserIdAsync` next to `GetByUserIdAsync`) rather than replacing it — internal callers that need the full collection (selection logic, tracker) keep working, while public endpoints use the paged variant. Update all fake repositories in the same pass so the test project builds. Confidence: 0.75
- When paginating a list endpoint whose only frontend consumers need a count (not the rows), do not depend on a capped page fetch (e.g., pageSize=100 still caps at 100); add a lightweight count helper that reuses an existing aggregate/summary endpoint (e.g., `getSecurityCount()` via the dashboard summary) so count displays stay correct for unbounded data. Confidence: 0.70
- Page-size defaults and clamps should not be repeated as inline literals in every controller/repository and frontend call site — centralize them in a configuration-bound options class (e.g., `PaginationOptions` with `DefaultPageSize`/`MaxPageSize` in `appsettings.json`, following the existing `ImportOptions`/`DeliveryOptions` pattern) so "records per page" is tunable without code edits. The user asked whether page settings are configurable, then explicitly approved implementing exactly this approach ("Alright, implement it") with env-var wiring in docker-compose and a note in AGENTS.md. Confidence: 0.85
